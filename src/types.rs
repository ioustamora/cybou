...
impl App {
    ...
    pub fn init_windows(&mut self) {
        use crate::windows::{WindowType, WindowState};

        // Initialize all window types with default states
        self.windows.insert(WindowType::Main, WindowState::default());
        self.windows.insert(WindowType::TextEncryption, WindowState::default());
        self.windows.insert(WindowType::FileEncryption, WindowState::default());
        self.windows.insert(WindowType::DigitalSignatures, WindowState::default());
        self.windows.insert(WindowType::PasswordTools, WindowState::default());
        self.windows.insert(WindowType::BackupManagement, WindowState::default());
        self.windows.insert(WindowType::CloudStorage, WindowState::default());
        self.windows.insert(WindowType::KeyManagement, WindowState::default());
        self.windows.insert(WindowType::Settings, WindowState::default());
        self.windows.insert(WindowType::FolderEncryption, WindowState::default());
    }

    /// Open a specific window type
    pub fn open_window(&mut self, window_type: crate::windows::WindowType) {
        if let Some(window_state) = self.windows.get_mut(&window_type) {
            window_state.is_open = true;
        }
    }

    /// Close a specific window type
    pub fn close_window(&mut self, window_type: crate::windows::WindowType) {
        if let Some(window_state) = self.windows.get_mut(&window_type) {
    ...
    pub fn is_window_open(&self, window_type: &crate::windows::WindowType) -> bool {
        self.windows.get(window_type)
    ...
    pub fn run(self) -> Result<(), Box<dyn std::error::Error>> {
        // TODO: Initialize Slint windows and run the application
        // This will be implemented when we integrate the Slint UI files
        Ok(())
    }
}
